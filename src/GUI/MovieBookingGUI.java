...
public class MovieBookingGUI extends JPanel implements ActionListener, ListSelectionListener
{
     ...
     */
    public MovieBookingGUI(ArrayList movies)
    {
        ...
        for (int row = 0; row < this.seatRowAmount; row++)
        {
            for (int col = 0; col < this.seatColAmount; col++)
            {
                ...
                seat.append(MovieSession.convertIndexToRow(row));
                seat.append(col);
    ...
    public void updateSeats()
    {
        ...
        {
            MovieSession movie = (MovieSession) this.moviePlayingList.getSelectedValue();
            for (int row = 0; row < this.seatRowAmount; row++)
            {
                for (int col = 0; col < this.seatColAmount; col++)
                {
                    this.seatingButtons[row][col].setBorder(this.lineBorder);
                    if (movie.getSeat(MovieSession.convertIndexToRow(row), col) instanceof AdultReservation)
                    {
                        this.seatingButtons[row][col].setEnabled(false);
                        this.seatingButtons[row][col].setBackground(this.adultColor);
                        this.seatingButtons[row][col].setForeground(this.tertiaryColor);
                    }
                    else if (movie.getSeat(MovieSession.convertIndexToRow(row), col) instanceof ElderlyReservation)
                    {
                        this.seatingButtons[row][col].setEnabled(false);
                        this.seatingButtons[row][col].setBackground(this.elderlyColor);
                        this.seatingButtons[row][col].setForeground(this.primaryColor);
                    }
                    else if (movie.getSeat(MovieSession.convertIndexToRow(row), col) instanceof ChildReservation)
                    {
                        this.seatingButtons[row][col].setEnabled(false);
        ...
        {
            selectedRow = MovieSession.convertRowToIndex(this.currentReservation.get(counter).getRow());
            ...
            for (int col = 0; col < this.seatColAmount; col++)
            {
                if (source == this.seatingButtons[row][col])
                {
                    if (this.adultButton.isSelected() || this.elderlyButton.isSelected() || this.childButton.isSelected())
                    {
                        ...
                        {
                            reservation = new AdultReservation(MovieSession.convertIndexToRow(row), col);
                            reservation.setComplementary(complementary);
                            this.currentReservation.add(reservation);
                        }
                        else if (this.elderlyButton.isSelected())
                        {
                            reservation = new ElderlyReservation(MovieSession.convertIndexToRow(row), col);
                            reservation.setComplementary(complementary);
                            this.currentReservation.add(reservation);
                        }
                        else if (this.childButton.isSelected())
                        {
                            reservation = new ChildReservation(MovieSession.convertIndexToRow(row), col);
                            reservation.setComplementary(complementary);
                            this.currentReservation.add(reservation);
                        }
                        updateSeats();
    ...
    public void valueChanged(ListSelectionEvent e)
    {
        ...
        if (this.moviePlayingList.getSelectedValue() != null)
        {
            ...
            MovieSession movie = (MovieSession) this.moviePlayingList.getSelectedValue();
            this.movieHeader.setText(movie.getMovieName());
            manageTicketPanel(false);
            manageMovieSeats(false); 
            this.newButton.setEnabled(true);
            this.currentReservation.clear();
    ...
    public static void main(String[] args)
    {
        ...
        MovieSession[] myMovieSession = new MovieSession[8];
        myMovieSession[0] = new MovieSession("SCP: The Movie", 'R', new Time(16, 30));
        myMovieSession[1] = new MovieSession("The Predator", 'M', new Time(4, 45, 30));
        myMovieSession[2] = new MovieSession("Justice League", 'G', new Time(4, 30));
        myMovieSession[3] = new MovieSession("Deadpool 2", 'R', new Time(4, 30));
        myMovieSession[4] = new MovieSession("Avengers: Infinity War", 'G', new Time(2));
        myMovieSession[5] = new MovieSession("Logan", 'R', new Time(7, 30));
        myMovieSession[6] = new MovieSession("Slenderman", 'M', new Time(2, 30, 25));
        myMovieSession[7] = new MovieSession("Despasito", 'M', new Time()); 
